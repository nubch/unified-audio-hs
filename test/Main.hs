...
main = hspec $ do
  ...
  --describe "TP8: Group adjustments preserve getters" $ do
  ...
  --describe "TP10: Group and channel panning" $ do
    --it "preserves base pans on both backends" $ do
      --runEff $ Fmod.runAudio TP.tp10
      --runEff $ SDL.runAudio TP.tp10
